...
if __name__ == '__main__':
    ...
    with open(os.path.join(root, '{}_train.txt'.format(name)), 'w') as file:
        for i in train_img_ids:
            file.write(i + '\n')
    print("build train file successfully, path is: {}, count is {}".format(os.path.join(root, '{}_train.txt'.format(name)), len(train_img_ids)))

    with open(os.path.join(root, '{}_val.txt'.format(name)), 'w') as file:
        for i in val_img_ids:
            file.writelines(i + '\n')
    print("build validate file successfully, path is: {}, count is {}".format(os.path.join(root, '{}_val.txt'.format(name)), len(val_img_ids)))
